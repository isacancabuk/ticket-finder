...
- **Multi-Domain & FIFA Checks:** Reliable API/HTML based scraping for European Ticketmaster domains (DE, ES, NL, BE, SE, CH, PL), UK, MX (alphanumeric IDs), and FIFA. FIFA integration leverages intelligent request throttling, 15-second minimum intervals, and dynamic cookie refreshes to avoid DataDome 403 Forbidden errors.
...
- **Queue-Based Ticket Availability:** When ticket prices are missing due to queuing or missing NEXT_DATA, the system accurately flags the ticket as found (using a special `-1` price flag) to prevent ignoring valid tickets, displaying "Price information unavailable" in the UI and Telegram notifications.
...
- `salesSite`: Optional selection of the target marketplace or resale site, displayed in the UI and Telegram notifications.
...
- **Card Grid:** Cards automatically filter by derived status strings (Tümü, Bulundu, Fiyat Aşıldı, Aranıyor, vb.). Includes a real-time status summary counter tracking query states.
- **Card Localization:** Sale pricing, found pricing, profit, and loss text are completely in Turkish. 
- **Filter Bar:** A robust filter bar exists for Order No/Event Name text search, Status, Platform, Country, and Sort Direction (including a profit-based sorting option), complete with a "Temizle" reset button.
...
- **Automated Authentication:** Expand the dynamic cookie generation and session refresh concepts (currently implemented for FIFA via Puppeteer) to other domains to prevent 401s and 403s on standard TM queues.
