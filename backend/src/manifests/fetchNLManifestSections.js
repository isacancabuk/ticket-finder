...
/**
 * Fetches the manifest for a Ticketmaster NL event and extracts
 * section code + name pairs.
 *
 * Sections are sorted by semantic priority:
 * 1. Floor/Standing/Special premium areas
 ...
 * @param {string} [opts.domain="nl"] - TLD for the TM domain (e.g. "nl")
 * @returns {Promise<{success: boolean, sections?: Array<{code: string, name: string}>, error?: string}>}
 */
export async function fetchNLManifestSections({ eventId, domain = "nl" }) {
  const url = `https://availability.ticketmaster.${domain}/api/v2/TM_NL/manifest/${eventId}`;
