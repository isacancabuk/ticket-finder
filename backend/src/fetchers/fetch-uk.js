...
export async function fetchUK({ eventId, section, minSeats = 1, maxPrice }) {
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    for (const pick of pagePicks) {
      ...
      // In section mode: section found — no need to check other offsets
      // (this section won't appear with different price across offsets)
      if (!isBroadMode) {
    ...
    // For section mode: exit as soon as we find a valid price for that section
    // For broad mode: continue searching all offsets
